...
export const extractPopulationData = async (
  ...
  try {
    ...
    const popStatsResult = db.exec(popStatsQuery, [raceId, gameId]) as Array<{
      columns: string[];
    ...
    const coloniesResult = db.exec(coloniesQuery, [raceId, gameId]) as Array<{
      columns: string[];
    ...
    const systemDistributionResult = db.exec(systemDistributionQuery, [
      raceId,
      gameId,
    ]) as Array<{
      columns: string[];
